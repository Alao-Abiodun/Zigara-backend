...
const orderModel = require("../../models/Payment/order.model");
const validateUser = require("../../utils/validations/index");
const User = require("../../models/Payment/order.model.js");
...
const paymentPlatform = async (req, res) => {
  ...
  if (!findUser)
    return errorResMsg(res, 400, { message: "User profile not found" });

  validateUser(paymentSchema);
  const paymentTypeArray = ["paystack", "flutterwave"];
  if (!paymentTypeArray.includes(req.body.paymentType.toLowerCase()))
    return errorResMsg(res, 400, "Please provide a valid payment type");
...
const paystackPayment = async (req, res, findUser) => {
  ...
  if (!findServiceOrder)
    return errorResMsg(res, 400, {
      message: "Please include a service order Id",
    });
...
const paystackVerify = async (req, res) => {
  ...
  request(options, async (error, response, body) => {
    ...
    if (!response.status)
      return errorResMsg(res, 400, {
        message: "An error occured with this transaction",
      });
    if (
      ...
      return errorResMsg(res, 400, {
        message: "Your transaction is not yet complete",
      });
    const findTxn = await orderModel.findOne({ reference });
    if (!findTxn)
      return errorResMsg(res, 400, { message: "Transaction cannot be found" });
